 これはプログラムではありません。BASIC!には200を超えるコマンドがあり、詳しい説明マニュアルが準備されています。
 以下の各コマンドの右端にある数字はそのマニュアルのページ数です。
 マニュアルは、右上のメニュー→『BASIC!について』から入手するか、サンプルプログラムf00a.download.manuaru.basで端末にダウンロードできます。
 2018/3現在、英語版のみです。日本語版の使い方については、右上のメニュー→『BASIC! 友の会ホームページ』を参照してください。
 アルファベットキーを押すと、その文字で始まるコマンドのところにスクロールします。(全ての端末でできるわけではありません)
